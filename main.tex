...
The most of works in Multi-Agent Systems doesn't not consider the possible enomous large scale of real applications, even if some exceptions exists~\cite{nguyen2018reinforcement}. In Aggregate Computing we need to write application \emph{scale free}, so we need to built 
also the learning algorithm and model able to work regardless of the system scale, that currently isn't considered in depth.
