...
Using these techniques brings near-optimum solutions to specific and complex tasks such as multiplayer games~\cite{DBLP:journals/nature/VinyalsBCMDCCPE19} and traffic control~\cite{DBLP:journals/aes/JinMK17}.
...
\end{enumerate*}
Aggregate Computing currently is supported by different toolkit as ScaFi~\cite{DBLP:conf/ecoop/CasadeiV16} and Protelis~\cite{DBLP:conf/sac/PianiniVB15}, giving us fast prototype capabilities. In this first part, we focused on practical results than foundational ones, hiding details of engineering hybrid systems like these.  
...
Besides, the collective behavior is cooperative and homogeneous namely each agent performs the same local behavior~\cite{DBLP:journals/aamas/PanaitL05}.  %%improve settings
%
...
From related works, we observe a current trend in applying standard Reinforcement Learning algoritms in distributed applications.
...
%
As the first attempt, we try to encode our gradient problem in terms of a learning problem and then using Q-Learning (or also the Monte Carlo Learning~\cite{DBLP:conf/nips/Thrun99}) to solve it.
...
\begin{itemize}
    ...
    define guidelines in merging these two approaches (from an engineering point of view), creating a sort of hybrid programming methodology.
    \item 3rd year: extends learning tasks to the framework level, closing the reality gap, making the deployment in real application efficient. Understand implications of this new way of programming, inserting learning ideas at the Aggregate Computing model level. 
\end{itemize}
